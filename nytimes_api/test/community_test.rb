...
class ComunityTest < Test::Unit::TestCase
  ...
  def test_byurl
    ...
    com = NYTimes::Community.new
    url = "matrix"
    match_type = "closest-stem-match" # "exact-match"
  
    #Simple call on json format
    records, header = com.by_url(match_type, url, {:format => "json"})
    assert_equal("http://api.nytimes.com/svc/community/v2//comments/url/#{match_type}.json?api-key=#{@api_key}&url=#{url}", "#{header[:url]}", "Invalid URL!")
    assert_equal(String, records.class, "Content return for JSON must be a String")
    
    #Simple call
    records, header = com.by_url(match_type, url)
    assert_equal("http://api.nytimes.com/svc/community/v2//comments/url/#{match_type}.xml?api-key=#{@api_key}&url=#{url}", "#{header[:url]}", "Invalid URL!")
    assert_equal(Array, records.class, "Content return for XML(default) must be an Array")

    #Simple call with offset
    records, header = com.by_url(match_type, url, {:offset => "25"})
    assert_equal("http://api.nytimes.com/svc/community/v2//comments/url/#{match_type}.xml?api-key=#{@api_key}&offset=25&url=#{url}", "#{header[:url]}", "Invalid URL!")
    assert_equal(Array, records.class, "Content return for XML(default) must be an Array")
